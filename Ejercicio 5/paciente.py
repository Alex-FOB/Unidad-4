...
class Paciente:
    ...
    def __init__(self, apellido, nombre, telefono, altura, peso):
        ...
        self.__telefono = self.validar(telefono, self.telefonoRegex, 'ERROR: teléfono es requerido')
        self.__altura = self.validar(altura, 'ERROR: altura es requerido')
        self.__peso = self.validar(peso, 'ERROR: peso es requerido')
    def getApe(self):
        return self.__ape
    def getNom(self):
        return self.__nom
